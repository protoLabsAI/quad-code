...
> {
  ...
  private async performRipgrepSearch(options: {
    ...
    // Add file exclusions from .gitignore and .protoignore
    ...
    if (filteringOptions.respectProtoIgnore) {
      // Load .protoignore from each workspace directory, not just the primary one
      ...
      for (const searchPath of paths) {
        ...
        const protoIgnorePath = path.join(dir, '.protoignore');
        if (
          !seenIgnoreFiles.has(protoIgnorePath) &&
          fs.existsSync(protoIgnorePath)
        ) {
