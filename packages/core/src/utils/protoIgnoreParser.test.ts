...
import { ProtoIgnoreParser } from './protoIgnoreParser.js';
import * as fs from 'node:fs/promises';
import * as path from 'node:path';
import * as os from 'node:os';

describe('ProtoIgnoreParser', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'protoignore-test-'));
  });
  ...
  describe('when .protoignore exists', () => {
    beforeEach(async () => {
    ...
    it('should ignore files specified in .protoignore', () => {
      const parser = new ProtoIgnoreParser(projectRoot);
  ...
  describe('when .protoignore does not exist', () => {
