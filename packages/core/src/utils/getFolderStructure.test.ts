...
`.trim();
  ...
  describe('with protoignore', () => {
    it('should ignore protoignore files by default', async () => {
      await fsPromises.writeFile(
        nodePath.join(testRootDir, '.protoignore'),
        'ignored.txt\nnode_modules/\n.gemini/\n!/.gemini/config.yaml',
      );
      await createTestFile('file1.txt');
      await createTestFile('node_modules', 'some-package', 'index.js');
    ...
    it('should not ignore files if respectProtoIgnore is false', async () => {
      await fsPromises.writeFile(
        nodePath.join(testRootDir, '.protoignore'),
        'ignored.txt\nnode_modules/\n.gemini/\n!/.gemini/config.yaml',
      );
